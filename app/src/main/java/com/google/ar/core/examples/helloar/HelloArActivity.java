...
public class HelloArActivity extends AppCompatActivity implements SampleRender.Renderer {
    ...
    private static final float MAX_DISTANCE_METERS = 2.0f;
    ...
    protected void onResume() {
        super.onResume();
        Log.d("DEBUG", "Step 2: onResume - AR session resumed");
        if (session == null) {
          /*  try {
                session = new Session(this);
            } catch (Exception e) {
...
//GUIDE
//  ARCORE -> Textures -> whenever asset, texture, rendering these issues. mostly ondraw or surface creation errors.
//  user interactions errors -> toast shoudl be on main thread -> action methods eg. handletap
//   logcat : fatal eg. FATAL EXCEPTION: GLThread 67


//  Copy paste the methods and ask for help to AI coding agent qwen,chatgpt: task,understand it
// Always push smaller changes to git
//
    ...
    public void onDrawFrame(SampleRender render) {
        ...
        Log.d("DEBUG", "Step 3: Camera state = " + camera.getTrackingState());
...
// Filter other horizontal planes relative to the floor
        for (Plane plane : session.getAllTrackables(Plane.class)) {
            if (plane.getType() == Plane.Type.HORIZONTAL_UPWARD_FACING
                    && plane.getTrackingState() == TrackingState.TRACKING) {

                float planeY = plane.getCenterPose().ty(); // height of this plane
                float heightAboveFloor = planeY - floorY;

                if (plane == floorPlane) {
                    showToastOnUiThread("Floor detected at height: " + planeY);
                } else {
                    if (heightAboveFloor < 0.8f) {
                        showToastOnUiThread("Table/Chair detected at " + heightAboveFloor + "m above floor");
                    } else {
                        showToastOnUiThread("Higher object detected");
                    }
                }
        ...
        // Visualize planes.
        // Filter only horizontal upward-facing planes
        // Filter only horizontal planes (floor + table/chair)
        ...
        for (Plane plane : session.getAllTrackables(Plane.class)) {
            if (plane.getType() == Plane.Type.HORIZONTAL_UPWARD_FACING
                ...
                // Draw only floor and table/chair planes (less than 0.8m above floor)
            ...
            for (HitResult hit : hitResultList) {
                ...
                if ((trackable instanceof Plane
                    ...
                    if (distance < MIN_DISTANCE_METERS) {
                        showToastOnUiThread("Object is too close!");
                        continue;
                    } else if (distance > MAX_DISTANCE_METERS) {
                        showToastOnUiThread("Object is too far!");
                        continue;
                    }
...
// For planes, check height relative to floor
                    if (trackable instanceof Plane) {
                        float heightAboveFloor = ((Plane) trackable).getCenterPose().ty() - floorY;

                        if (heightAboveFloor > 0.8f) {
                            showToastOnUiThread("Cannot place object on high plane!");
    ...
    /** Finds the lowest horizontal upward-facing plane (floor) currently tracked. */
    /**
     * Finds the lowest horizontal upward-facing plane (floor) currently tracked.
     */
    private Plane findFloorPlane() {
        ...
        if (floorPlane != null) {
            ...
            // placeAnchor(float x, float y, float z);



            final float FLOOR_TOLERANCE = 0.05f; // 5 cm
            for (Plane plane : session.getAllTrackables(Plane.class)) {
                if (plane.getType() == Plane.Type.HORIZONTAL_UPWARD_FACING
                        && plane.getTrackingState() == TrackingState.TRACKING) {
                    float planeY = plane.getCenterPose().ty();
                    float heightAboveFloor = planeY - minY;

                    if (Math.abs(heightAboveFloor) < FLOOR_TOLERANCE) {
                        showToastOnUiThread("Floor detected at height: " + String.format("%.2f", planeY) + "m");
                    } else if (heightAboveFloor < 0.8f) {
                        showToastOnUiThread("Table/Chair detected at " + String.format("%.2f", heightAboveFloor) + "m above floor");
                    } else {
                        showToastOnUiThread("Higher object detected at " + String.format("%.2f", heightAboveFloor) + "m");
                    }
                }
            }
        }
    ...
    private void logFloorPolygon() {
        try{
            ...
            Plane floorPlane = findFloorPlane();
            if (floorPlane == null) {
                Log.e(TAG, "No floor plane found");
                return;
            }

            FloatBuffer polygon = floorPlane.getPolygon();
            if (polygon == null || polygon.limit() == 0) {
                Log.e(TAG, "Floor plane polygon not available yet");
                return;
            }

            polygon.rewind();
            StringBuilder builder = new StringBuilder("Floor polygon: ");
            while (polygon.hasRemaining()) {
                float x = polygon.get();
                float y = polygon.get();
                float z = polygon.get();
                builder.append(String.format("(%.2f, %.2f, %.2f) ", x, y, z));

                Log.d("FloorPolygon123", builder.toString());
            }
        }
        catch (Exception e){
            Log.e(TAG, "Error in logFloorPolygon: " + e.getMessage());
        }
        // Log.d(TAG, builder.toString());
        // Log.d("FloorPolygon123", builder.toString());

    }
    private List<float[]> getFloorVertices() {
        List<float[]> vertices = new ArrayList<>();
        Plane floorPlane = findFloorPlane();
        if (floorPlane == null) return vertices;

        FloatBuffer polygon = floorPlane.getPolygon();
        if (polygon == null || polygon.limit() == 0) return vertices;

        polygon.rewind();
        while (polygon.remaining() >= 3) { // ensure at least 3 floats remain
            float x = polygon.get();
            float y = polygon.get();
            float z = polygon.get();
    ...
    private void computeBoundingBox() {
        List<float[]> vertices = getFloorVertices();
        ...
        Log.d("FloorGrid", "Bounds: X[" + minX + "," + maxX + "] Z[" + minZ + "," + maxZ + "]");
    }

    private void placeGridAnchors() {
        List<float[]> vertices = getFloorVertices();
        Log.d("FloorGrid", "Vertices count: " + vertices.size());
        if (vertices.isEmpty()) {
            showToastOnUiThread("Floor vertices not available yet");
            return;
        }
        ...
        for (float x = minX; x <= maxX; x += gridSize) {
            for (float z = minZ; z <= maxZ; z += gridSize) {
                float[] point = new float[]{x, floorY, z};

                //      if (isPointInsidePolygon(point, vertices)) {
                   /* Pose pose = Pose.makeTranslation(x, floorY, z);
                  Anchor anchor = session.createAnchor(pose);
                    wrappedAnchors.add(new WrappedAnchor(anchor, findFloorPlane()));*/
                //  placeAnchor(x, floorY, z);
                // Log point and whether it is inside polygon
                boolean inside = isPointInsidePolygon(point, vertices);
                Log.d("FloorGrid", "Testing point: " + Arrays.toString(point) + ", inside=" + inside);
    ...
    // Simple method to check if a point is inside a convex polygon (XZ plane)
    private boolean isPointInsidePolygon(float[] point, List<float[]> polygon) {
        int count = polygon.size();
        boolean result = false;
        for (int i = 0, j = count - 1; i < count; j = i++) {
            float xi = polygon.get(i)[0], zi = polygon.get(i)[2];
            float xj = polygon.get(j)[0], zj = polygon.get(j)[2];
    ...
    private void placeAnchor(float x, float y, float z) {
        if (session == null) {
            Log.e(TAG, "AR session is null. Cannot place anchor.");
            return;
        }
        ...
        try {
            ...
            for (float t : translation) {
                if (Float.isNaN(t) || Float.isInfinite(t)) {
                    Log.e(TAG, "Invalid pose. Skipping anchor placement.");
                    return;
                }
            }

            Anchor anchor = session.createAnchor(pose);
            wrappedAnchors.add(new WrappedAnchor(anchor, null));

        } catch (Exception e) {
            Log.e(TAG, "Failed to create anchor safely", e);
        }
    }
